...
class EASHelper {
    ...
    async attestDeliveryProof(proof, recipient, options) {
        ...
        const encodedData = abiCoder.encode(['bytes32', 'bytes32', 'uint256', 'string', 'uint256', 'string'], [
            proofData.txId,
    ...
    async verifyDeliveryAttestation(txId, attestationUID) {
        ...
        if (isRevoked) {
            ...
            throw new Error(`Attestation has been revoked: ${attestationUID} (revoked at timestamp ${revokedAt})`);
        }
        // 5. Check expiration
        ...
        try {
            ...
            if (Number(timestamp) > maxFutureTime) {
                throw new Error(`Decoded timestamp is in far future: ${timestamp} (current time: ${now})`);
            }
    ...
    async verifyAndRecordForRelease(txId, attestationUID) {
        ...
        if (!this.attestationTracker.isValidForTransaction(attestationUID, txId)) {
            throw new Error(`Attestation ${attestationUID} cannot be used for transaction ${txId}. ` +
