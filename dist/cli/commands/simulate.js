...
exports.runSimulatePay = runSimulatePay;
...
function createSimulateCommand() {
    const cmd = new commander_1.Command('simulate')
...
// simulate pay
// ============================================================================
function createSimulatePayCommand() {
...
async function runSimulatePay(to, amount, options, output) {
    ...
    if (options.json) {
        ...
        output.print('');
        output.print(output_1.fmt.bold('Transaction Details:'));
        ...
        output.print('');
        output.print(output_1.fmt.bold('Fee Breakdown (1% + $0.05 min):'));
        ...
        if (feeCalculation.minimumApplied) {
            output.print(output_1.fmt.yellow('  (Minimum fee applied - amount < $5)'));
        }
        output.print('');
        output.print(output_1.fmt.bold('Requirements:'));
        ...
        if (validation.warnings && validation.warnings.length > 0) {
            output.print('');
            output.print(output_1.fmt.yellow('Warnings:'));
            for (const warning of validation.warnings) {
                output.print(output_1.fmt.yellow(`  - ${warning}`));
            }
        }
        output.print('');
...
// simulate fee
// ============================================================================
function createSimulateFeeCommand() {
    return new commander_1.Command('fee')
 ...
 */
class SimulationAdapter extends BaseAdapter_1.BaseAdapter {
     ...
     */
    validatePayment(to, amount, deadline) {
        const errors = [];
        const warnings = [];
        let parsedAmount;
        let parsedDeadline;
